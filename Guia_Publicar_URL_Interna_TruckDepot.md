...
> **⚠️ ACTUALIZACIÓN (agosto 2026):** el buscador de direcciones ahora usa la **API de Geocoding de Google Maps** (ya no Mapbox). El proyecto se publica en **Railway** y la clave de Google **no va dentro del HTML**: vive únicamente en el servidor, como variable de entorno `GOOGLE_MAPS_API_KEY` (pestaña *Variables* del servicio en Railway). La búsqueda de direcciones la hace el servidor a través del endpoint `/geocode`, así que los agentes no ven ni ingresan ninguna clave. En Google Cloud, la clave puede restringirse por **API** (solo *Geocoding API*), pero **no por sitio web/URL** (las peticiones salen del servidor, no de una página web; si quieres restringir por origen, usa restricción por IP con la IP de salida de Railway). Los pasos de abajo describen el método anterior con Mapbox (archivo estático con token incrustado) y se conservan solo como referencia.
