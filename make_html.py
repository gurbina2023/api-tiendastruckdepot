...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  .ctab .tab.on{ background:var(--rojo); border-color:var(--rojo); }
...
<header>
  ...
  <div class="sub">Verificador de cobertura de entrega</div>
  <div class="stats" id="stats"></div>
</header>
<div class="wrap">
  <div id="side">
    <div class="tabbar ctab" id="countrybar">
      <div class="tab on" data-c="gt" onclick="setCountry('gt')">Guatemala</div>
      <div class="tab" data-c="cr" onclick="setCountry('cr')">Costa Rica</div>
    </div>
    <div class="tabbar">
...
const DATA = /*DATA*/;
/* países disponibles: límites aproximados para validar coordenadas pegadas */
const COUNTRY_CFG={
  gt:{name:'Guatemala',  center:[15.3,-90.4], zoom:7, bounds:[13,18.5,-92.9,-88], hint:'latitud ~14–17 y longitud ~-88 a -92'},
  cr:{name:'Costa Rica', center:[9.8,-84.2],  zoom:8, bounds:[8,11.3,-86,-82.5],  hint:'latitud ~8–11 y longitud ~-83 a -86'}};
let CUR='gt';
const BR=()=>DATA.branches.filter(b=>b.country===CUR);
const COLOR = {}; DATA.branches.forEach(b=>COLOR[b.name]=b.color);
const norm = s => (s||"").normalize('NFD').replace(/[̀-ͯ]/g,'').toLowerCase().replace(/\s+/g,' ').trim();
function updStats(){ document.getElementById('stats').textContent =
  COUNTRY_CFG[CUR].name + ' · ' + BR().length + " sucursales · " + DATA.index.filter(it=>it.country===CUR).length + " ubicaciones"; }

/* ---------- selector de país ---------- */
function setCountry(c){
  if(c===CUR || !COUNTRY_CFG[c]) return;
  document.querySelectorAll('#countrybar .tab').forEach(x=>x.classList.toggle('on',x.dataset.c===c));
  if(map) BR().forEach(b=>map.removeLayer(branchLayers[b.name]));
  CUR=c;
  if(map) BR().forEach(b=>map.addLayer(branchLayers[b.name]));
  buildLegend(); updStats();
  if(map && selMarker){ map.removeLayer(selMarker); selMarker=null; }
  document.getElementById('addr').value=''; document.getElementById('q').value='';
  document.getElementById('matches').innerHTML=''; document.getElementById('branchInfo').innerHTML='';
  const co=document.getElementById('coords'); if(co) co.value='';
  document.getElementById('result').innerHTML='<div class="card muted">Ahora estás viendo <b>'+COUNTRY_CFG[c].name+'</b>.</div>';
  fitCountry();
}
function fitCountry(){ if(!map) return;
  const fs=DATA.geo.features.filter(f=>f.properties.country===CUR);
  try{ map.fitBounds(L.geoJSON({type:'FeatureCollection',features:fs}).getBounds().pad(0.05)); }
  catch(e){ const c=COUNTRY_CFG[CUR]; map.setView(c.center,c.zoom); } }
...
function initMap(){
  ...
  DATA.branches.forEach(b=>{ branchLayers[b.name]=L.layerGroup(); if(b.country===CUR) branchLayers[b.name].addTo(map); });
...
function buildLegend(){
  ...
  BR().forEach(b=>{ const o=document.createElement('option'); o.value=b.name; o.textContent=b.name; sel.appendChild(o); });
}
function allLayers(on){ BR().forEach(b=>{ const ck=document.getElementById('ck_'+norm(b.name).replace(/[^a-z0-9]/g,''));
...
function onType(){
  ...
  const hits=DATA.index.filter(it=>it.country===CUR && norm(it.place).includes(q)).slice(0,40);
...
function selectPlace(it){
  ...
  const locals=it.entries.filter(e=>e.minutes<=720);
...
function showBranch(){
  ...
  if(!order.length){ html+='<div class="muted" style="margin-top:6px">Esta tienda aún no tiene promesas de entrega a domicilio registradas (venta y recogida en tienda).</div>'; }
...
function searchAddress(){
  ...
  fetch('geocode?q='+encodeURIComponent(q)+'&country='+CUR)
   ...
   .then(function(o){
     ...
     if(!o.d || !o.d.found){ box.innerHTML='<div class="card muted">Google Maps no encontró esa dirección en '+COUNTRY_CFG[CUR].name+'. Agrega detalle (zona, municipio, provincia/departamento).</div>'; return; }
...
function openGoogle(){ const q=document.getElementById('addr').value.trim(); if(!q){ document.getElementById('result').innerHTML='<div class="card muted">Escribe primero la dirección del cliente.</div>'; return; }
  window.open('https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(q+' '+COUNTRY_CFG[CUR].name),'_blank'); }
...
function checkCoords(){ const box=document.getElementById('result');
  ...
  const B=COUNTRY_CFG[CUR].bounds;
  const inC=(a,b)=>a>B[0]&&a<B[1]&&b>B[2]&&b<B[3];
  if(!inC(lat,lng)){ if(inC(lng,lat)){ const t=lat; lat=lng; lng=t; }
    else { box.innerHTML='<div class="card muted">Esas coordenadas no parecen estar en '+COUNTRY_CFG[CUR].name+'. Debe ser '+COUNTRY_CFG[CUR].hint+'.</div>'; return; } }
...
function showPoint(lat,lng,title){
  ...
  const hits={}; DATA.geo.features.forEach(f=>{ const p=f.properties; if(p.kind!=='coverage'||p.country!==CUR)return;
  ...
  let near=null,nd=1e9; BR().forEach(b=>{ const d=Math.hypot(b.store[0]-lat,b.store[1]-lng); if(d<nd){nd=d;near=b;} });
...
function quickOSM(){ const q=document.getElementById('addr').value.trim(); const box=document.getElementById('result');
  ...
  fetch('https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes='+CUR+'&q='+encodeURIComponent(q))
